...
const query = ref(props.barcode || ''), category = ref(''), selectedWarehouse = ref(props.warehouse || ''), rows = ref<any[]>([]), total = ref(0), start = ref(0), error = ref(''), busy = ref(false), appending = ref(false)
...
const receiveUom = props.movementKind === 'Receive' ? props.boot.uoms.find((u: any) => u.name === 'Nos')?.name || '' : props.boot.uoms[0]?.name || ''
const item = ref({ item_name: '', item_group: props.boot.item_groups[0]?.name || '', stock_uom: receiveUom, barcode: props.barcode || '', description: '', has_batch_no: false, has_expiry_date: false })
...
async function search(offset = 0, append = false) {
  ...
  } catch (e: any) { error.value = e.message } finally { if (seq === sequence) appending.value = false }
}
function drawerScroll(event: Event) {
  if (typeof IntersectionObserver !== 'undefined') return
  const target = event.currentTarget as HTMLElement
  if (target.scrollHeight - target.scrollTop - target.clientHeight < 180 && rows.value.length < total.value && !busy.value && !appending.value) void search(rows.value.length, true)
}
...
async function create() {
  ...
  if (!props.boot.uoms.some((u: any) => u.name === item.value.stock_uom)) { error.value = '请选择已有单位或先新建单位'; return }
...
onMounted(async () => {
  ...
  if (typeof IntersectionObserver !== 'undefined') {
    observer = new IntersectionObserver(entries => {
      if (entries.some(entry => entry.isIntersecting) && rows.value.length < total.value && !busy.value && !appending.value) void search(rows.value.length, true)
...
<template>
<div class="drawer-backdrop"><aside ref="drawer" class="drawer wide" role="dialog" aria-modal="true" aria-label="选择物品" @scroll.passive="drawerScroll"><button class="drawer-close" @click="emit('close')">×</button>
