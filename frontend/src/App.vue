...
const showSettings = () => route.path === '/settings'
...
<template><FrappeUIProvider><main v-if="checking" class="app-shell"><p>正在检查库存系统配置…</p></main><main v-else-if="!ready && !showSettings()" class="app-shell"><header><h1>物资管理尚未就绪</h1></header><p>请完成以下配置后再使用库存功能：</p><ul><li v-for="issue in issues" :key="issue.code">{{issue.message}}</li></ul><button v-if="canRepair" class="primary" :disabled="repairBusy" @click="repair">{{repairBusy?'正在修复…':'自动修复'}}</button><p>如需手动配置，请打开：</p><p><a href="/settings">物资管理设置</a> · <a href="/app/company" target="_blank">ERPNext 公司</a> · <a href="/app/warehouse/view/tree" target="_blank">ERPNext 仓库树</a> · <a href="/app/stock-settings" target="_blank">库存设置</a></p><button @click="check">重新检查</button><p v-if="error" class="error">{{error}}</p></main><RouterView v-else/><div v-if="sessionExpired" class="modal auth-modal" role="alertdialog" aria-modal="true"><section><h2>请重新登录</h2><p>未保存的输入仍保留在此窗口。请在新窗口登录后返回继续保存。</p><a href="/login?redirect-to=%2Finventory%2Fauth-complete" target="_blank" rel="noopener">打开 Frappe 登录</a><button @click="resume">继续保存</button><p v-if="error" class="error">{{error}}</p></section></div><div v-if="updateAvailable" class="pwa-update" role="alert"><span>发现新版本，保存完成后可以更新。</span><button class="primary" @click="applyUpdate">更新并重新加载</button></div></FrappeUIProvider></template>
