...
const INFRASTRUCTURE_LABELS = new Set(['寺院仓库', '实体库房', '虚拟库房', '借出', '损坏待处理', '未定位'])
...
function fallbackOf(row: WarehouseRecord): WarehouseFallbackRole { return row.fallback_role === 'room_default' || row.fallback_role === 'group_default' ? row.fallback_role : null }
...
export function presentWarehouse(row: WarehouseRecord, rows: WarehouseRecord[] = []): PresentedWarehouse {
  ...
  const semanticType = typeOf(row), fallbackRole = fallbackOf(row), storedLabel = clean(text(row.stored_label || row.warehouse_name || row.name), row)
  ...
  const breadcrumb = text(row.breadcrumb) || chain.filter(label => !INFRASTRUCTURE_LABELS.has(label)).join(' / ') || localLabel
...
export function presentWarehouses(rows: WarehouseRecord[]): PresentedWarehouse[] {
  return rows.map(row => presentWarehouse(row, rows)).filter(row =>
    row.fallbackRole === null && row.canFilter && !INFRASTRUCTURE_LABELS.has(row.storedLabel),
  )
}
...
export function warehousePresentation(name: string, rows: WarehouseRecord[]): PresentedWarehouse {
  const row = rows.find(item => item.name === name)
  return row ? presentWarehouse(row, rows) : presentWarehouse({ name }, rows)
}
export function warehouseFilterOptions(rows: WarehouseRecord[], counts: Record<string, number> = {}) {
  return presentWarehouses(rows).map(row => ({
    ...row.raw,
    label: row.localLabel,
    search_text: `${row.name} ${row.localLabel} ${row.breadcrumb} ${row.storedLabel}`.toLowerCase(),
    parent: row.parentName || undefined,
    filter_value: row.filterValue,
    operation_value: row.operationValue,
