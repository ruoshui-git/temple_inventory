    ...
    <template v-else-if="boot?.is_manager">
      ...
      <div class="settings-layout">
        ...
        <section v-if="selectedWarehouse" class="settings-detail" aria-live="polite">
          <h3>{{ label(selectedWarehouse.name) }}</h3>
      ...
      <form @submit.prevent="action('configure_warehouse', fresh)">
        ...
        <label>上级仓库<select v-model="fresh.parent_warehouse"><option v-for="warehouse in parentOptions" :key="warehouse.name" :value="warehouse.name">{{ label(warehouse.name) }}</option></select></label>
