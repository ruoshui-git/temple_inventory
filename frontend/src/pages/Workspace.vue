...
const picker = ref(false), scanner = ref(false), unknown = ref(''), scanBusy = ref(false), recentScans = ref<string[]>([])
...
const isIssue = computed(() => form.value?.movement_kind === 'Issue')
  ...
  try {
    ...
    if (route.params.name) d = await workspaceApi('load_workspace', { name: route.params.name })
      ...
      d = { name: '', revision: 0, stock_entry: null, docstatus: 0, sync_error: '', attachments: [], data: {
        ...
        source_type: '', donor_source: '', purpose: '', recipient: '', activity: '', loan_reference: '', notes: '', signature: '', items: [], sections: [], from_warehouse: '', to_warehouse: '', lease_program_warehouse: ''
      }}
    }
...
async function showReview() {
  ...
  if (!form.value.signature) { error.value = '请完成负责人签名'; return }
...
<div v-if="!readonly" class="toolbar workspace-tools"><button type="button" @click="picker=true;unknown=''">＋添加物品</button><button type="button" @click="scanner=!scanner">▣ 连续扫码</button><span v-if="isIssue && lastScannedWarehouse" class="filter-chip">出库仓库：{{label(lastScannedWarehouse)}} <button type="button" @click="lastScannedWarehouse=''">清除</button></span></div>
...
<div v-if="review" class="modal"><section><h2>确认{{labels[form.movement_kind]}}</h2><p>{{postingTimeMode==='manual' ? `${form.posting_date} ${form.posting_time}` : '提交时使用当前时间'}}</p><p>{{form.donor_source||form.recipient||''}} · {{form.purpose||''}}</p><p>{{form.items.length}} 行物品</p><p v-for="(qty,uom) in totals">{{qty}} {{uom}}</p><p v-for="g in groups">{{label(g.location)}} · {{g.lines.length}} 行</p><p>活动：{{form.activity||'无'}}</p><p>附件：{{record.attachments?.length||0}}</p><p>负责人：{{form.responsible_person}}</p><p>{{form.signature?'✓ 已签名':'尚未签名'}}</p><p v-if="error" class="error">{{error}}</p><button :disabled="confirming" @click="review=false">返回修改</button><button class="primary" :disabled="confirming" @click="confirm">{{confirming?'正在确认…':`确认${labels[form.movement_kind]}`}}</button></section></div>
</template></main>
</template>
