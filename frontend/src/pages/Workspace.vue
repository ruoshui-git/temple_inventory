...
const catalog = ref<Record<string, any>>({}), people = ref<any[]>([]), activities = ref<any[]>([])
...
function invalidate() { if (form.value) { form.value.recorder_signature = ''; form.value.reviewer_signature = '' } }
function setNoIndependentReviewer(value: boolean) { if (!form.value) return; form.value.no_independent_reviewer = value; if (value) { form.value.reviewer_name = ''; form.value.reviewer_note = ''; form.value.reviewer_signature = '' }; invalidate(); changed(true) }
function setBorrowerSameAsReviewer(value: boolean) { if (!form.value) return; form.value.borrower_same_as_reviewer = value; if (value && form.value.reviewer_name) form.value.borrower = form.value.reviewer_name; invalidate(); changed(true) }
  ...
  try {
    ...
    if (route.params.name) d = await workspaceApi('load_workspace', { name: route.params.name })
      ...
      d = { name: '', revision: 0, stock_entry: null, docstatus: 0, sync_error: '', attachments: [], data: {
        movement_kind: kind, posting_date: `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`, posting_time: `${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`, posting_time_mode: 'current', responsible_person: boot.value.user,
        activity: '', notes: '', source_text: '', purpose_text: '', borrower: '', recorded_by: boot.value.user, reviewer_name: '', borrower_same_as_reviewer: false, no_independent_reviewer: false, reviewer_note: '', recorder_signature: '', reviewer_signature: '', loan_record: '', return_record: '', loss_record: '', items: [], sections: [], from_warehouse: '', to_warehouse: ''
      }}
    ...
    currentLocation.value = physical[0]?.name || ''; currentRoom.value = roomFor(currentLocation.value, tree.value); currentTo.value = boot.value.settings.loan_warehouse || boot.value.settings.leased_warehouse || boot.value.settings.default_lease_program_warehouse || ''
    await hydrate()
    if (!d.name) { dirty.value = true; queue.schedule(true) }
...
async function selectItem(item: any) {
  if (isIssue.value) {
    ...
    line.value = { id: crypto.randomUUID(), item_code: item.item_code, qty: 1, uom: item.stock_uom, warehouse, from_warehouse: warehouse, to_warehouse: currentTo.value, batch_no: '', new_batch: false }
  } else {
    catalog.value[item.item_code] = item; chosen.value = item; picker.value = false; unknown.value = ''; editingIndex.value = -1
    line.value = { id: crypto.randomUUID(), item_code: item.item_code, qty: 1, uom: item.stock_uom, warehouse: currentLocation.value, from_warehouse: currentLocation.value, to_warehouse: form.value.movement_kind === 'Loan' ? (boot.value.settings.loan_warehouse || boot.value.settings.leased_warehouse || boot.value.settings.default_lease_program_warehouse) : form.value.movement_kind === 'Damage' ? boot.value.settings.damaged_warehouse : currentTo.value, batch_no: '', new_batch: false }
  }
  await loadBatches()
}
...
async function attach(event: Event) { const input = event.target as HTMLInputElement; error.value = ''; try { await queue.flush(); if (dirty.value) return; for (const file of Array.from(input.files || [])) await upload(file, 'Inventory Workspace', record.value.name); record.value = await workspaceApi('load_workspace', { name: record.value.name }) } catch (e: any) { error.value = e.message } finally { input.value = '' } }
...
async function showReview() {
  ...
  if (!form.value.responsible_person) { error.value = '请选择负责人'; return }
  if (!form.value.recorder_signature) { error.value = '请完成记录人签名'; return }
...
async function confirm() {
  ...
  try {
    ...
    if (form.value.movement_kind === 'Return') {
      ...
      const candidate = outstanding.find((row:any) => row.borrower && row.borrower === form.value.borrower)
      if (candidate && window.confirm('该借用方仍有未结数量。是否继续记录遗失？')) await router.push(`/new/Loss?loan_item=${encodeURIComponent(candidate.loan_item)}`)
    }
  } catch (e: any) { error.value = e.message } finally { confirming.value = false }
}
...
<section class="details-panel"><button v-if="!readonly" type="button" @click="detailsOpen=!detailsOpen">{{detailsOpen?'收起详细信息':'添加详细信息'}}</button><div v-if="detailsOpen || readonly" class="details-content"><fieldset :disabled="readonly||confirming" @input="invalidate"><div class="form-grid"><label v-if="isReceive">来源<input v-model="form.source_text" placeholder="例如：捐赠、采购或内部调拨"></label><label v-if="!isReceive">用途<input v-model="form.purpose_text" list="purposes"><datalist id="purposes"><option v-for="p in ['分发','活动/演出','内部使用','对外捐赠','损坏/报废','其他']">{{p}}</option></datalist></label><label v-if="['Loan','Return','Loss'].includes(form.movement_kind)">借用方<input v-model="form.borrower" :readonly="form.movement_kind==='Return' && !!form.items?.length" placeholder="姓名、单位或团体"></label><label>活动<button type="button" class="selector-button" @click="activityDialog=true">{{activities.find((a:any)=>a.name===form.activity)?.title||'选择活动'}}</button></label><label>备注<textarea v-model="form.notes"/></label></div></fieldset><section class="attachments"><h2>附件 / 照片 · {{record.attachments?.length||0}}</h2><template v-if="!readonly"><label class="file-button">上传文件<input type="file" multiple @change="attach"></label></template><div class="attachment-grid"><article v-for="f in record.attachments" :key="f.name"><a :href="f.file_url" target="_blank" rel="noopener"><img v-if="/\.(png|jpe?g|webp|gif)$/i.test(f.file_name)" :src="f.file_url" class="thumb">{{f.file_name}}</a><button v-if="!readonly" type="button" @click="removeFile(f.name)">移除</button></article></div></section></div></section>
<fieldset :disabled="readonly||confirming"><label>记录人 <input :value="form.recorded_by || boot.user" readonly></label><label>负责人 <span v-html="requiredMark"/><select v-model="form.responsible_person" required><option v-for="p in people" :value="p.name">{{p.full_name||p.name}}</option></select></label><label v-if="form.movement_kind==='Loan'"><span>借用方与鉴证人相同</span><input type="checkbox" :checked="form.borrower_same_as_reviewer" @change="setBorrowerSameAsReviewer(($event.target as HTMLInputElement).checked)"> 与鉴证人相同</label></fieldset><label class="checkbox"><input type="checkbox" :checked="form.no_independent_reviewer" @change="setNoIndependentReviewer(($event.target as HTMLInputElement).checked)" :disabled="readonly||confirming"> 无独立鉴证人</label><template v-if="!form.no_independent_reviewer"><label>鉴证人 <input v-model="form.reviewer_name" @input="changed(true)"></label><label>鉴证说明 <input v-model="form.reviewer_note" @input="changed(true)"></label><SignaturePad v-model="form.reviewer_signature" :disabled="readonly||confirming" @complete="changed(true)"/></template><SignaturePad v-model="form.recorder_signature" :disabled="readonly||confirming" @complete="changed(true)"/><button v-if="!readonly" class="primary confirm-button" :disabled="confirming||conflict" @click="showReview">查看确认摘要</button><p v-if="record.stock_entry">库存记录：{{record.stock_entry}} <a v-if="boot.is_manager" :href="`/app/stock-entry/${encodeURIComponent(record.stock_entry)}`">管理员查看</a></p>
...
<div v-if="review" class="modal"><section><h2>确认{{labels[form.movement_kind]}}</h2><p>{{postingTimeMode==='manual' ? `${form.posting_date} ${form.posting_time}` : '提交时使用当前时间'}}</p><p>{{form.source_text||form.purpose_text||form.borrower||''}}</p><p>{{form.items.length}} 行物品</p><p v-for="(qty,uom) in totals">{{qty}} {{uom}}</p><p v-for="g in groups">{{label(g.location)}} · {{g.lines.length}} 行</p><p>活动：{{form.activity||'无'}}</p><p>附件：{{record.attachments?.length||0}}</p><p>记录人：{{form.recorded_by || boot.user}}</p><p>鉴证人：{{form.no_independent_reviewer ? '无独立鉴证人' : (form.reviewer_name || '未填写')}}</p><p>{{form.recorder_signature?'✓ 已签名':'尚未签名'}}</p><p v-if="error" class="error">{{error}}</p><button :disabled="confirming" @click="review=false">返回修改</button><button class="primary" :disabled="confirming" @click="confirm">{{confirming?'正在确认…':`确认${labels[form.movement_kind]}`}}</button></section></div>
