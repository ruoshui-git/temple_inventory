...
<p v-if="signatureState('handler').status === 'stale'" class="error">签名仍保留，但内容已更改，请重新确认后提交。<button type="button" @click="reconfirmSignature('handler')">重新确认经手人签名</button></p></form><LoanItemPicker v-if="loanPicker" :tree="tree" @select="selectLoanItem" @close="loanPicker=false"/><ItemPicker v-if="picker" :boot="boot" :barcode="unknown" :stock-only="isIssue" :warehouse="lastScannedWarehouse" :posting-date="postingTimeMode==='manual' ? form.posting_date : undefined" :posting-time="postingTimeMode==='manual' ? form.posting_time : undefined" @warehouse-change="lastScannedWarehouse=$event" @select="selectItem" @close="picker=false;unknown=''"/>
