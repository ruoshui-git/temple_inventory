...
async function uploadAttachmentFiles(files: File[]) { error.value = ''; try { if (!record.value.name) { dirty.value = true; queue.schedule(true) }; await queue.flush(); if (dirty.value || !record.value.name) return; for (const file of files) await upload(file, 'Inventory Workspace', record.value.name); record.value = await workspaceApi('load_workspace', { name: record.value.name }) } catch (e: any) { error.value = e.message } }
...
<template>
<main class="app-shell workspace"><header><button type="button" @click="close">‹ 首页</button><h1>{{labels[form?.movement_kind]||'库存记录'}}</h1><button v-if="!readonly && record?.name" type="button" @click="deleteDraft">删除草稿</button><span role="status">{{readonly ? (record.docstatus===1?'已完成 ✓':'已取消') : saveStatus}}</span></header>
<p v-if="error" class="error" role="alert">{{error}}</p><div v-if="conflict" class="error">记录已在其他窗口修改。当前输入仍保留在页面中，请复制需要保留的内容后重新打开记录。<button type="button" @click="load">重新加载</button></div><button v-else-if="dirty && !saving" type="button" @click="queue.schedule(true)">重试保存</button>
<LoadingIndicator v-if="!form || !boot" text="正在加载工作区…" /><template v-else><form @submit.prevent="showReview">
<fieldset :disabled="readonly || confirming"><div class="form-grid"><label>日期 <span v-html="requiredMark"/><input type="date" v-model="form.posting_date" :required="postingTimeMode==='manual'" @input="markManualTime"></label><label>时间 <span v-html="requiredMark"/><input type="time" step="1" v-model="form.posting_time" :required="postingTimeMode==='manual'" @input="markManualTime"></label><button v-if="postingTimeMode==='manual'" type="button" @click="setCurrentTime">使用当前时间</button><p v-else class="field-hint">提交时使用当前时间</p></div></fieldset>
...
<section class="details-panel"><button v-if="!readonly" type="button" @click="detailsOpen=!detailsOpen">{{detailsOpen?'收起详细信息':'添加详细信息'}}</button><div v-if="detailsOpen || readonly" class="details-content"><fieldset :disabled="readonly||confirming" @input="invalidate"><div class="form-grid"><label v-if="isReceive">来源<input v-model="form.source_text" placeholder="例如：捐赠、采购或内部调拨"></label><label v-if="!isReceive">用途<input v-model="form.purpose_text" list="purposes"><datalist id="purposes"><option v-for="p in ['分发','活动/演出','内部使用','对外捐赠','损坏/报废','其他']">{{p}}</option></datalist></label><label v-if="['Return','Loss'].includes(form.movement_kind) || (form.movement_kind==='Loan' && !form.borrower_is_handler_or_witness)">借用方 <span v-if="form.movement_kind==='Loan'" v-html="requiredMark"/><input v-model="form.borrower" :required="form.movement_kind==='Loan' && !form.borrower_is_handler_or_witness" :readonly="form.movement_kind==='Return' && !!form.items?.length" placeholder="姓名、单位或团体"></label><label>活动<button type="button" class="selector-button" @click="activityDialog=true">{{activities.find((a:any)=>a.name===form.activity)?.title||'选择活动'}}</button></label><label>备注<textarea v-model="form.notes"/></label></div></fieldset></div></section>
<AttachmentList v-if="record" class="workspace-attachment-list" :attachments="record.attachments" :editable="!readonly" @upload="uploadAttachmentFiles" @remove="removeAttachmentFile" /><fieldset :disabled="readonly||confirming"><label v-if="form.movement_kind==='Loan'" class="checkbox"><input type="checkbox" :checked="form.borrower_is_handler_or_witness" @change="setBorrowerDeclaration(($event.target as HTMLInputElement).checked)"> 借用方是经手人或鉴证人</label><label>经手人 <span v-html="requiredMark"/><input v-model="form.handler_name" required placeholder="请输入姓名或称谓" @input="changed(true)"></label><SignaturePad label="经手人签名" v-model="form.handler_signature" :required="true" :disabled="readonly||confirming" @complete="changed(true)"/><label class="checkbox"><input type="checkbox" :checked="form.no_independent_reviewer" @change="setNoIndependentReviewer(($event.target as HTMLInputElement).checked)" :disabled="readonly||confirming"> 无独立鉴证人</label><template v-if="!form.no_independent_reviewer"><label>鉴证人 <span v-html="requiredMark"/><input v-model="form.reviewer_name" required @input="changed(true)"></label><SignaturePad label="鉴证人签名" v-model="form.reviewer_signature" :required="true" :disabled="readonly||confirming" @complete="changed(true)"/></template></fieldset><button v-if="!readonly" type="submit" class="primary confirm-button" :disabled="confirming||conflict">提交</button><small v-if="!readonly" class="muted">系统记录用户：{{ form.recorded_by || boot.user }}</small><p v-if="record.stock_entry">库存记录：{{record.stock_entry}} <a v-if="boot.is_manager" :href="`/app/stock-entry/${encodeURIComponent(record.stock_entry)}`">管理员查看</a></p>
<p v-if="signatureState('handler').status === 'stale'" class="error">签名仍保留，但内容已更改，请重新确认后提交。<button type="button" @click="reconfirmSignature('handler')">重新确认经手人签名</button></p></form><LoanItemPicker v-if="loanPicker" :tree="tree" @select="selectLoanItem" @close="loanPicker=false"/><ItemPicker v-if="picker" :boot="boot" :barcode="unknown" :movement-kind="form.movement_kind" :stock-only="isIssue" :warehouse="lastScannedWarehouse" :posting-date="postingTimeMode==='manual' ? form.posting_date : undefined" :posting-time="postingTimeMode==='manual' ? form.posting_time : undefined" @warehouse-change="lastScannedWarehouse=$event" @select="selectItem" @close="picker=false;unknown=''"/>
