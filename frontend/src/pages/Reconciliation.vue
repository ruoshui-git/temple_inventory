...
import { api, upload, workspaceApi, warehouseLabel } from '../lib/api'
...
const rows = ref<any[]>([]), notes = ref(''), batchChoices = ref<any[]>([]), batchItem = ref<any>()
...
async function confirm() { await persist(); if (error.value || !record.value?.name) return; try { saving.value = true; conflict.value = false; adopt(await workspaceApi('confirm_reconciliation', { name: record.value.name, revision: record.value.revision })); window.dispatchEvent(new Event('ti:refresh-shell')) } catch (cause: any) { error.value = cause.message; conflict.value = String(cause.message || '').includes('账面数量') } finally { saving.value = false } }
