...
async function scan(value: string) { scanner.value = false; search.value = value; await nextTick(); await find() }
...
function showReview() { if (!rows.value.some(row => row.count_state === 'counted' || row.count_state === 'not_found')) { error.value = '请至少确认一行盘点数量'; return }; review.value = true }
...
<template>
  ...
  <main class="app-shell wide-shell reconciliation-page"><header><button type="button" @click="close">‹ 返回</button><h1>{{ readonly ? '盘点记录' : '盘点' }}</h1><span role="status">{{ saving ? '正在保存…' : record?.stock_reconciliation ? '已完成' : record?.name ? '已保存' : '尚未保存' }}</span></header>
  <LoadingIndicator v-if="loading" text="正在加载盘点…"/><template v-else><p v-if="error" class="error">{{ error }} <button type="button" @click="load">重试</button></p><template v-if="!error"><form @submit.prevent="showReview"><section class="reconciliation-scope"><h2>1. 选择盘点范围</h2><label>实体叶子库位<span v-if="!readonly" class="required-mark" aria-hidden="true">*</span><select v-model="warehouse" :disabled="readonly" :required="!readonly" @change="persist"><option value="" disabled>请选择库位</option><option v-for="node in leaves" :key="node.name" :value="node.name">{{ warehouseLabel(node.name, boot?.warehouse_tree || []) }}</option></select></label><label>盘点方式<select v-model="mode" :disabled="readonly" @change="persist"><option value="selective">抽查盘点</option><option value="whole">整库盘点（需逐项确认）</option></select></label><p class="field-hint">盘点时间：{{ postingDate }} {{ postingTime }}。整库盘点必须逐项选择“已盘点”或“未找到，计为 0”。</p></section><section><h2>2. 录入实点数量</h2><div v-if="!readonly" class="result-toolbar"><input v-model="search" placeholder="搜索物品或条码" @keydown.enter.prevent="find"><button type="button" @click="find">搜索</button><button type="button" @click="scanner = true">扫描</button></div><div v-if="results.length" class="selection-row"><button v-for="item in results" :key="item.item_code" type="button" @click="add(item)">{{ item.item_name }} · {{ item.item_code }}</button></div><div class="reconciliation-lines"><article v-for="row in rows" :key="`${row.item_code}:${row.batch_no || ''}`" class="item-card"><div><b>{{ row.item_name || row.item_code }}</b><small>{{ row.item_code }}<template v-if="row.batch_no"> · 批次 {{ row.batch_no }}</template> · {{ row.uom }} · 账面 {{ row.ledger_qty }}</small></div><label>实点数量<input v-model="row.counted_qty" type="number" min="0" step="any" :readonly="readonly" @change="row.count_state = 'counted'; persist()"></label><button v-if="!readonly && mode === 'whole'" type="button" @click="markNotFound(row)">未找到，计为 0</button><strong v-if="row.counted_qty !== ''">差异 {{ Number(row.counted_qty) - Number(row.ledger_qty) }}</strong><small v-if="mode === 'whole'">状态：{{ row.count_state === 'not_found' ? '未找到，计为 0' : row.count_state === 'counted' ? '已盘点' : '待确认' }}</small></article></div><p v-if="!rows.length" class="empty-state">尚未添加盘点物品</p></section><section class="reconciliation-summary"><h2>3. 检查差异</h2><p>已确认 {{ countedRows.length }} 行 · 差异 {{ discrepancyRows.length }} 行</p><p v-for="(summary, uom) in uomSummary" :key="uom">{{ uom || '未指定单位' }}：增加 {{ summary.increase }}，减少 {{ summary.decrease }}</p></section><section class="attachments"><h2>备注</h2><textarea v-model="notes" :readonly="readonly" placeholder="可选的盘点说明" @change="persist"></textarea></section><AttachmentList v-if="record" class="reconciliation-attachment-list" :attachments="record.attachments" :editable="!readonly" @upload="uploadAttachmentFiles" @remove="removeAttachmentFile" /><section class="reconciliation-audit"><h2>盘点责任确认</h2><label>经手人<span v-if="!readonly" class="required-mark" aria-hidden="true">*</span><input v-model="audit.handler_name" :readonly="readonly" :required="!readonly" placeholder="请输入姓名或称谓" @change="persist"></label><SignaturePad label="经手人签名" v-model="audit.handler_signature" :required="!readonly" :disabled="readonly" @complete="persist"/><label v-if="!readonly"><input v-model="audit.no_independent_reviewer" type="checkbox" @change="persist"> 无独立鉴证人</label><p v-else>{{ audit.no_independent_reviewer ? '无独立鉴证人' : '有独立鉴证人' }}</p><template v-if="!audit.no_independent_reviewer"><label>鉴证人<span v-if="!readonly" class="required-mark" aria-hidden="true">*</span><input v-model="audit.reviewer_name" :readonly="readonly" :required="!readonly" @change="persist"></label><SignaturePad label="鉴证人签名" v-model="audit.reviewer_signature" :required="!readonly" :disabled="readonly" @complete="persist"/></template></section><div v-if="!readonly" class="submit-row"><button type="button" @click="persist">保存盘点</button><button class="primary" type="submit" :disabled="saving || !rows.some(row => row.count_state === 'counted' || row.count_state === 'not_found')">提交</button></div></form></template></template><Scanner v-if="scanner" presentation="modal" @scan="scan" @close="scanner = false"/>
  </main>
</template>
